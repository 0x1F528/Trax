...
class Trax extends Function {                                           // Trax is a function with behavior added
    ...
    static UPDATE = Symbol("update");                                   // standard behavior when change detected
    static PROCESS = {                                                  // flag to maintain the context of the current processing traversal
    ...
    static _activeElems = new Set();                                    // this is where we store the active nodes that will respond to UPDATE events
    ...
    _notify(event, arg) {                                               // notify _subs of s that it has changed
        ...
        if (event === Trax.UPDATE) {                                    // standard Flow: a publisher has been updated and all subscribers are being notified
        ...
        } else {                                                        // alternate Flow: a custom event has been fire()d and this event will be propagated to subscribers recursively
            ...
            if (typeof this[name] === 'function') {                     // if there is a function that matches the event, ...
                let res = this[name](this.value, arg, this);            // call it as trax.event(current-value, event-args, this-trax)
        ...
        if (this.onChange && this.onChange())                           // if there is a captured onchange function or true boolean, ...
    ...
    _evaluateValue(fct, values, processLevel) {                         // what is the value of t?
        ...
        if (processLevel !== Trax.PROCESS.REFRESH && this.onChange && (onChange = this.onChange()) && onChange instanceof Function ) {
            onChange.call(this, this.value, this);                      //    ... call it to notify of change (value, this)
        }
        return res;
    }
    ...
    /* Trouble shooting support methods: */
    _logger(from, to) {                                                 // log the UPDATE event
        if (this.id && this.id()) {                                     // we will may the logging function if there is an ID on this trax element
            if (this.log())                                             // element level log function
                this.log()(this.id(), from?.toString(), to?.toString());
            else if (Trax._logger)                                      // Trax level log function
                Trax._logger(this.id(), from?.toString(), to?.toString());
        }
        return to;
    };

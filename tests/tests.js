import { trax, Trax } from '../modules/trax.js'
...
((descr = "event signal") => {
  ...
  var a = trax(3).id('a');
  var b = trax(a).id('b');
  var c = trax(b).id('c').onChange((x) => { changeClosure = x; }).handler(FOO, (val, arg) => { eventClosure = [val, arg]});
...
((descr = "event signal with modification") => {
  ...
  var a = trax('pot').id('a').handler(APPEND, (val, arg, t) => { t(val + arg); return Trax.HALT; })
  var b = trax(a).id('b');
  var c = trax(b).id('c').onChange((x) => { changeClosure = x; }).handler(APPEND, (val, arg) => { eventClosure = [val, arg]});
...
((descr = "logging") => {
    log(descr);
    var closure = [];
    var logger = (id, from, to) => { closure.push( [id, from, to] ) };
    ...
    assert_equality(closure[0].join('|'), ['a',undefined,'3'].join('|'), descr + " (1.1)");
    assert_equality(closure[1].join('|'), ['b',undefined,'3'].join('|'), descr + " (1.2)");
    assert_equality(closure[2].join('|'), ['c',undefined,'3'].join('|'), descr + " (1.3)");

    a(15);
    assert_equality(closure.length, 6, descr + " (2)");
    assert_equality(closure[3].join('|'), ['a',undefined,'15'].join('|'), descr + " (2.1)");
    assert_equality(closure[4].join('|'), ['b','3','15'].join('|'), descr + " (2.2)");
    assert_equality(closure[5].join('|'), ['c','3','15'].join('|'), descr + " (2.3)");
})();
